...
2. **Infer the user's intent from the article itself.** A `note` is optional. When one is absent, read the piece and work out on your own which ticker(s) and which profile wave it bears on, and whether it reads as a positive or negative signal, exactly as you would for anything you surfaced in your own searches. A bare URL with no note is a complete, valid input; do not ask the user to explain why they sent it.
3. **Evidence, not an override.** A user article raises a candidate to your attention; it does not force an add or a remove. You still make the yes/no call, and every guardrail (listing date, liquidity floor, wave-bucket diversity, exclusions) still applies unchanged. If the article argues for a ticker that violates a guardrail, do not add it, and note the conflict in `rationale_overall`.
4. **Cite it when it matters.** If a user article drives or supports an add or remove, include it in that decision's `news_evidence` with its real URL and date, exactly like a source you found yourself. If after reading you conclude it changes nothing, say so in one clause of `rationale_overall` so the user can see you read it and why it did not move you.
...
2. **WebSearch hygiene.** Append `before:{as_of_date_plus_one}` to every query (the leakage guard: never read anything dated after the decision date). For *news/recency* queries — every Pass-1 discovery beat and every Pass-2 ticker news search — also append `after:{as_of_date_minus_lookback}`, where the lookback is `recent_news_lookback_days` (90 days for a quarterly rebalance). This makes each news query a trailing window ending at the decision date, e.g. a 2025-09-30 quarterly call searches `... after:2025-07-01 before:2025-10-01`, so you read the recent quarter rather than all of the year. **Exception:** structural lookups that need full history — listing-date / "{ticker} IPO date" / "ETF launch date" checks — take `before:` only (no `after:`), since you want to know if the security existed at any point up to `as_of_date`. For every result: if no date is shown, discard; if the date is after `as_of_date`, discard. Cite only material that survives both filters.
...
- *Profile-wave beats.* Run at least one plain discovery query for **every wave named in `investor_profile.md`'s "Strategy & beliefs" section — no exceptions, including the slow-moving or non-technology ones** (e.g. aging-population demographics, energy/geopolitical). Do not skip a wave just because it is quiet this month or has no obvious ticker; the point is to give every declared wave a chance to surface a name. The profile's waves are the source of truth, so these terms evolve automatically as the user edits their thesis. Examples: "rockets & spacecraft" → `space stocks`, `rocket launch stocks`; "nuclear" → `nuclear stocks`, `SMR stocks`; "robotics" → `robotics stocks`, `humanoid robot stocks`; "quantum computing" → `quantum computing stocks`; energy/geopolitical → `geopolitics`, `shipping stocks`, `tanker stocks`; defense/rearmament → `defense stocks`, `defense ETF`; aging-population demographics → `healthcare stocks`, `eldercare REIT stocks`, `aging population demographics stocks`. Also consult `news_sources.md`, whose sources are grouped by these same waves.
...
- *Coverage check.* Before moving to Pass 2, confirm that every wave named in the profile has at least one discovery beat in your `search_terms`. If a wave is missing, add its beat and run it. This is a hard requirement, not best-effort.
...
**Pass 2 — ticker-keyed due-diligence (drives KEEPs and REMOVEs).** For each current-watchlist ticker and each candidate surfaced in Pass 1, search the ticker by name and gather 2 to 4 dated bullets from major financial news outlets (Bloomberg, Reuters, WSJ, FT, CNBC, the company's own newsroom, SEC filings). Prefer these over speculative blog posts. (In backtest mode both passes obey the as-of-date discipline: the trailing-window `after:...before:...` filters and the suppression list apply to discovery beats too.)
