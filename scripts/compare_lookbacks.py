...
For each LB in LOOKBACKS, runs a 12-month monthly-rebalance walk-forward
on the 12-ticker watchlist with mean_variance λ=1, recomputing μ and Σ
from a window of length LB years at each rebalance. Wave-stage tilts
come from data/wave_history.csv (same as the headline backtest).

Sweep values are capped at 1.3 years because NUKZ launched 2024-01-24,
making the 12-ticker price matrix simultaneously complete only since
then. Longer lookbacks silently collapse to ~16 months of common
coverage, so values above 1.3y would all produce duplicate results.
...
LOOKBACKS_YEARS = [0.25, 0.5, 0.75, 1.0, 1.3]
LAMBDA = 1.0
...
def run_walk_forward(prices: pd.DataFrame, daily_dates, lookback_years: float,
                     wh_df: pd.DataFrame) -> pd.Series:
    """Walk-forward backtest with this lookback. μ and Σ are estimated
    from the most recent ``lookback_years`` of prices ending at each
    monthly rebalance."""
    shares = None
    values = []
...
# Fetch enough history to cover the longest lookback at the start of the window.
...
print(f"price-matrix common coverage starts: {prices.index[0].date()}")
...
for lb in LOOKBACKS_YEARS:
    print(f"running walk-forward, lookback={lb}y ...")
    curves[lb] = run_walk_forward(prices, daily_dates, lb, wh_df)
...
for i, lb in enumerate(LOOKBACKS_YEARS):
    # short-lookback (momentum-chasing, noisy) = warm red; long-lookback
    # (smooth, value-tilted) = cool blue.
    t = i / max(n - 1, 1)
    r = int(220 * (1 - t)); g = int(80 + 30 * t); b = int(60 + 195 * t)
    color = f"rgb({r},{g},{b})"
    s = curves[lb]
    fig.add_trace(go.Scatter(x=s.index, y=s.values, mode="lines",
                             name=f"lookback={lb}y", line={"width": 2, "color": color},
...
table_html = (
    ...
    f"Sweep is capped at 1.3y because NUKZ launched 2024-01-24; longer "
    f"lookbacks would collapse to the same ~16-month effective window."
    f"</p>"
)
...
chart_caption = (
    ...
    f"<i>Walk-forward 12-month backtest run five times, once per lookback "
    ...
    f"estimating μ and Σ at each monthly rebalance. SPY rebased to share the "
    f"starting value. Orange dotted vertical lines mark rebalance dates.</i>"
    f"</p>"
)
...
print()
print(f"{'LB':>6} {'Final':>11} {'Return':>9} {'Vol':>7} {'Sharpe':>7} {'MaxDD':>8} {'vs SPY':>9}")
for lb in LOOKBACKS_YEARS:
    s = stats[lb]
    print(f"{lb:>5.2f}y {s['final']:>11,.0f} {s['return']*100:>+8.2f}% {s['vol']*100:>6.2f}% "
