...
#   3. Sunday 19:00 + Monday 08:00  scripts/review_curation.sh -> biweekly CBS + CFT curation + report (--if-due)
...
# entry is missing.
#
# Entry 3 fires Sunday evening so the fresh recommendation is ready before a Monday-morning rebalance, and
# again Monday 08:00 purely as a catch-up for a Sunday the machine slept through. --if-due makes the second
# firing a no-op whenever Sunday's already ran, so the pair costs one curator call per portfolio per period.
#
