...
- **Optimizer (two configs)**: the **live/robust** config is mean_variance λ=0.5, lookback 1.5y, concentration_cap 0.70 (the profile defaults `/review-portfolio` uses with real money). The **published dashboard** runs an **aggressive sweep-optimal** config — λ=0.33, lookback 0.5y, concentration_cap 1.0 — set as backtest-only overrides in `investor_profile.md`'s `backtest` section so they never touch the live path. Cadence quarterly, max_watchlist_size 8 in both.
...
- **Output**: 13 distinct tickers passed through the watchlist (adds: RKLB, QTUM, URA, ITA, BOTZ, NUKZ, STNG, CEG); final watchlist `[MSFT, NVDA, RKLB, QTUM, ITA, NUKZ, CEG, BOTZ]` spans six wave buckets (AI, rockets, robotics, quantum, nuclear, defense).

| Strategy | Final ($50K start) | Return | Active vs SPY |
|---|---|---|---|
| **Curator — published/aggressive** (λ=0.33, lookback 0.5y, cap=1.0) | **$458,310** | **+816.6%** | **+629.4pp** |
| Curator — live/robust (λ=0.5, lookback 1.5y, cap=0.70) | $268,792 | +437.6% | +250.3pp |
| Buy-and-hold starter (equal-weight, then hold) | $143,628 | +187.3% | +128.6pp |
| SPY benchmark (rebased) | $79,345 | +58.7% | — |

The published dashboard renders the **aggressive** row. It is the sweep-optimal ceiling, not the honest expectation: with `concentration_cap=1.0` and a 0.5y momentum lookback the optimizer ends up **~100% in a single name (RKLB)** — maximal overfit to the one 2025 winner (−48% max drawdown). The **live/robust** row is what `/review-portfolio` recommends with real money, and is the figure to trust.

Optimizer settings: `λ=0.5`, `lookback=1.5y`, `concentration_cap=0.70`, `max_watchlist_size=8` (all from `investor_profile.md` defaults). The buy-and-hold baseline is an equal-weight allocation (20% in each of AAPL/MSFT/GOOGL/NVDA/SPY) bought on day 0 and held without rebalancing. NVDA is in the starter so the curator gets no credit for the obvious AI winner. With NVDA already in the buy-and-hold, the **robust** config's remaining lift is **+250pp** (≈25.6pp annualized, 59.8% annualized return, −43.2% max drawdown; ends 70% RKLB / 30% WELL) — from the curator's thematic adds (rockets, robotics, quantum, nuclear, defense, demographics) plus quarterly re-weighting. The aggressive config's bigger +629pp lift is almost entirely the single RKLB bet the unconstrained cap allows (ends ~100% RKLB).

**Execution lag.** The replay models the gap between a rebalance *signal* (decided on the rebalance date's close) and the *trade* (placed later, since a live user runs a review and only acts afterward). The CLI flag `--t-update-days` (default **1**, i.e. next session; also set in `investor_profile.md`'s `backtest` section) sets this lag in trading days; the one-time initial capital deployment is not lagged. The figures here are the robust config at t=1. **On this shorter window the lag is material**: the optimistic same-close run (`t_update_days=0`) returns +458.2%, so a one-session execution delay trims the curator's edge by ~21pp (the lift over buy-and-hold shrinks from ~271pp to +250pp). That is the honest case for publishing the t=1 number — it is what a normally-paced human actually captures, not a fast-execution upper bound. (On the longer 2021–2026 run the lag was near-noise; here the smaller rebalance count and sharper thematic entries make timing matter more.) Set `--t-update-days 0` to reproduce the optimistic same-close run.
...
1. *The result is fragile (n=1).* The curator trailed the equal-weight buy-and-hold for ~85% of the window and only pulled ahead in 2025, when the optimizer concentrated heavily in RKLB and RKLB ran — ~70% under the live cap=0.70, a full **100%** under the aggressive published cap=1.0. The lift rests on a single late position, so it cannot distinguish skill from luck; the aggressive config simply levers that one bet harder.
...
To reproduce the published numbers and refresh `docs/backtest_curator.html`: `python -m src.cli backtest --curator-runs-dir data/curator_runs/postcovid --out-dir data/backtest_curator_postcovid --benchmarks SPY`. The optimizer knobs come from `investor_profile.md`'s `backtest` section (the aggressive overrides 0.33 / 0.5y / cap=1.0 drive the published dashboard); pass `--max-weight 0.70 --risk-aversion 0.5` only to reproduce the live/robust figure instead. Replays the saved JSONs through the optimizer in a few seconds. Re-running the curator agents from scratch costs another ~$3. The longer **2021–2026 run** (+1267% over its full 5y window, including the 2021 melt-up) is preserved in `data/curator_runs/5y-sweep-cap08/` + `data/backtest_curator_5y/` for reference.
