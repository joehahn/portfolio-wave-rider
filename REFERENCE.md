...
|---|---|---|---|
| **Curator** (λ=0.67, lookback 0.5y, cap=0.80) | **$375,101** | **+650.2%** | **+591.5pp** |
...
1. *The result is fragile (n=1).* Under the current config the curator leads the equal-weight buy-and-hold most of the time — ahead on ~74% of trading days, worst shortfall only ≈ −8%, permanently ahead from 2024-10-10 (the short 0.5y lookback drives this; the longer 1.5y lookback led on only ~26% of days with a −39% worst shortfall). But the final lift concentrates in the single RKLB position (the optimizer rode it near the 80% cap as it ran in 2024–2025), so the magnitude rests on one dominant bet and the backtest alone cannot distinguish skill from luck — which is exactly what forward testing is meant to resolve.
...
To reproduce the published numbers and refresh `docs/backtest_curator.html`: `python -m src.cli backtest --curator-runs-dir data/curator_runs/postcovid --out-dir data/backtest_curator_postcovid --benchmarks SPY`. The optimizer knobs default to the live config from `investor_profile.md` (`λ=0.67 / lookback=0.5y / cap=0.80`); pass `--max-weight` / `--risk-aversion` / `--lookback-years`, or set overrides in the profile's `backtest` section, only to test a different candidate config. Replays the saved JSONs through the optimizer in a few seconds. Re-running the curator agents from scratch costs another ~$3. The longer **2021–2026 run** (+1267% over its full 5y window, including the 2021 melt-up) is preserved in `data/curator_runs/5y-sweep-cap08/` + `data/backtest_curator_5y/` for reference.
