...
# computes two extra baselines (fixed-watchlist same cadence; buy-and-hold of starter)
# for comparison. Writes snapshots.csv, recommendations.csv, baselines_totals.csv,
# curation_summary.json, and report.md to the out_dir.
...
| `docs/backtest_curator.html` | Curator-backtest dashboard: equity-curve race (curator vs fixed-rebalance vs buy-and-hold vs SPY) plus watchlist Gantt timeline. | One-off; refresh by re-running `dashboard --curator-backtest-dir` |
...
- **Curator**: 20 strict-as-of-date Sonnet calls, each with WebSearch `before:` filters, suppression list from `scripts/post_date_events.py`, and a self-critique pass. Total cost ~$3, total wall clock ~6 min (5 batches of 4 parallel calls).
- **Output**: 26 distinct tickers entered the watchlist over the run (with adds and removes); final watchlist spans all six named wave buckets

| Strategy | Final ($50K start) | Return | Active vs SPY |
|---|---|---|---|
| **Curator-driven** | **$117,731** | **+135.46%** | **+57.3pp** |
| Buy-and-hold starter (day-0 optimize then hold) | $101,844 | +103.69% | +25.5pp |
| Fixed watchlist rebalance (same cadence, no curation) | $90,111 | +80.22% | +2.1pp |
| SPY benchmark (rebased) | $89,073 | +78.15% | — |
