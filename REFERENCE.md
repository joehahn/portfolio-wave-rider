...
Candidate next steps, in rough build order. Each would live on an experimental branch and race against the curator+MVO 5y baseline before touching the live flow.

1. **Momentum rotation layer.** Insert a pure-Python ranking step between the curator and the optimizer: the curator maintains a wider universe (cap 30-50), then a volatility-adjusted trailing-return score (per-ticker `mean(r)/σ(r)` over a tunable lookback) picks the top N for mean-variance weighting. Cross-sectional momentum at 3-12 month formation windows is the documented anomaly; sweep the lookback walk-forward, same hygiene as the concentration-cap sweep.

2. **Multi-source evidence fusion.** Add parallel specialist agents that feed the curator primary-source signals alongside news: SEC EDGAR Form 4 insider buys, USASpending.gov contract awards (directly relevant to the space/nuclear/quantum waves), and prediction-market probabilities (Kalshi API) for macro regime context. All three are free, API-accessible, and citable; add one at a time and measure whether curator picks improve.

3. **Pooled cross-sectional return model.** Supervised learning on the (ticker, month) panel: price-trend and evidence features predicting forward *relative* return (demeaned per date), trained across ~2,000 tickers with time-blocked, embargoed validation. This is the honest version of the "RL across ticker batches" idea: batches of tickers are not independent games because they share one macro history, and since holding a stock doesn't change its future returns the Q-function degenerates to a regression, so use a regressor. Survivorship bias in yfinance ticker lists is the main data risk; the design note on the RL degeneracy is itself a deliverable.
