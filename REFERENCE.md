...
.venv/bin/python -m src.cli analyze --tickers AAPL MSFT NVDA --period 1.5y --max-weight 0.80
...
|---|---|---|---|
| **Curator** (λ=0.33, lookback 1.5y, cap=0.80) | **$312,558** | **+525.1%** | **+466.4pp** |
...
1. *The result is fragile (n=1).* The curator trailed the equal-weight buy-and-hold for most of the window and only pulled clear in 2025, when the optimizer concentrated near the cap in RKLB (ends ~80% RKLB at cap=0.80) and RKLB ran. The lift rests largely on that single late position, so the backtest alone cannot distinguish skill from luck — which is exactly what forward testing is meant to resolve.
...
To reproduce the published numbers and refresh `docs/backtest_curator.html`: `python -m src.cli backtest --curator-runs-dir data/curator_runs/postcovid --out-dir data/backtest_curator_postcovid --benchmarks SPY`. The optimizer knobs default to the live config from `investor_profile.md` (`λ=0.33 / lookback=1.5y / cap=0.80`); pass `--max-weight` / `--risk-aversion` / `--lookback-years`, or set overrides in the profile's `backtest` section, only to test a different candidate config. Replays the saved JSONs through the optimizer in a few seconds. Re-running the curator agents from scratch costs another ~$3. The longer **2021–2026 run** (+1267% over its full 5y window, including the 2021 melt-up) is preserved in `data/curator_runs/5y-sweep-cap08/` + `data/backtest_curator_5y/` for reference.
