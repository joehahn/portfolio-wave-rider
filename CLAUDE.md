...
The current 5y backtest (Mar 2021 → Mar 2026, starter watchlist of AAPL/MSFT/GOOGL/NVDA/SPY, 21 quarterly curator calls, profile defaults `λ=0.5 / lookback=1.5y / max_weight=0.7 / max_watchlist_size=8`) lifts realized return to **+1259.1%** vs **+328.7%** for an equal-weight buy-and-hold of the starter and **+75.7%** SPY, with a −45.6% max drawdown. NVDA is in the buy-and-hold starter so the curator doesn't get credit for picking the obvious 5y winner — the +34.8pp/yr remaining lift comes from the curator's other thematic adds (nuclear, robotics, rockets, quantum) plus the optimizer's quarterly re-weighting. Rendered at [docs/backtest_curator.html](https://joehahn.github.io/portfolio-wave-rider/backtest_curator.html). The full setup and reproducibility notes live in `REFERENCE.md` and `data/backtest_curator_5y/report.md`. The canonical run dir is `data/curator_runs/5y-sweep-cap08/`; `5y-quarterly/` is the historical record from when the default was cap=12.
