...
- Skills (`.claude/skills/`): four slash commands.
  - `/initialize-portfolio` (one-shot): reads the profile, produces a thesis-driven dollar allocation across the watchlist, calls `init-holdings` to convert dollars to shares (which writes both `holdings.csv` real positions and `watchlist.csv` the curator universe), runs `snapshot --force`, persists the allocation to `data/thesis_baseline.json`, and writes a thesis-only report. No optimizer, no news. Refuses to run if holdings already has positions or thesis_baseline.json already exists.
