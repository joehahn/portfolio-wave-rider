...
Three cron scripts drive the forward loop. Every dashboard refresh they run is **render-only** (no LLM, no API cost); the only LLM calls in cron are the two curations in the 19:00 job.
...
3. `scripts/review_curation.sh`, **weekdays at 19:00 local**: the biweekly curation, one `scripts/run_bootstrap_curator.py --if-due` per paper portfolio (CBS, then CFT with `--forward-only --blend-backtest-news --report`). This is the only cron step that calls the curator LLM. It runs at 19:00 so it lands AFTER the 18:30 news pull and curates on same-day news; `--if-due` self-gates to the profile's `rebalance_period` (biweekly), so firing every weekday is safe and the cadence stays profile-driven (no cron edit to change it). The old live `cli review --if-due` step is RETIRED (2026-08-01) because it curated the root `watchlist.csv` in parallel with CFT and produced a second, competing recommendation.

Users install them by pasting three lines into `crontab -e` (README §6 has the block, using a `PWR_path` variable); `scripts/install_cron.sh` is an optional helper that adds all three idempotently. All scripts resolve their own location, are pure-bash, log to `data/snapshot.log`, and work on macOS and Linux.
