...
- Dashboards are static HTML under `docs/`, all git-tracked and served by GitHub Pages from `main/docs/`. Four are cron-refreshed (see Automation): `docs/index.html` (CFT, the site landing page), `docs/curator_bootstrap.html` (CBS), `docs/retrieval_bootstrap.html` (RBS), `docs/retrieval_forward.html` (RFT). The rest are built on demand by the render scripts in `scripts/` (backtest, sweep, diagnostics, pool browser). `docs/forward_dashboard.html` is a redirect stub kept so old links land on `index.html`; its builder `scripts/build_forward_dashboard.py` is deleted. cron does not push, but a Stop hook does: `.claude/settings.local.json` runs `scripts/autopush_docs.sh` at the end of every Claude Code session, staging and pushing root `*.md`, `docs/*.html`, and `data/reports/*-curation.md`. Outside a Claude session, `git add docs/ && git commit && git push` is the manual publish step. Pages deploys via GitHub Actions (`.github/workflows/pages.yml`, `build_type: workflow`), which uploads `docs/` as-is and skips the legacy Jekyll builder — a push touching `docs/**` triggers the deploy; watch it under the repo's Actions tab. (The old legacy branch builder was flaky, failing with instant "Page build failed" errors; `docs/.nojekyll` is kept as belt-and-suspenders.)
...
- `data/thesis_baseline.json`: the day-0 allocation. Schema: `{date, allocations_usd, reasoning, holdings}`. Written once at portfolio inception (originally by the retired `/initialize-portfolio` skill; nothing writes it now) and read-only thereafter. `build_dashboard` reads its `date` to scope the (now retired) live dashboard's time-series charts; the read is guarded, so a missing file degrades gracefully rather than crashing. The old second reader, `scripts/build_forward_dashboard.py`, is deleted.
...
Three cron scripts drive the forward loop. Every dashboard refresh they run is **render-only** (no LLM, no API cost); the only LLM calls in cron are the two curations in the weekly curation job.

1. `scripts/news_pull.sh`, **daily at 18:30 local incl. weekends**: `cli pull-news` extends `data/forward_corpus/`, then two retriever dashboards refresh off that fresh corpus, `scripts/build_bootstrap_dashboard.py` (RBS, Retriever Bootstrap) and `scripts/build_forward_retrieval_dashboard.py` (RFT, Retriever Forwardtest). Both live here rather than in the snapshot job because they visualize the corpus this job just wrote.
2. `scripts/price_snapshot.sh`, **weekdays at 16:30 local**: `cli snapshot --force` (one authoritative close row per date), then the two paper-portfolio dashboards refresh off the fresh prices, both via `scripts/refresh_cbs.py`: CBS to `docs/curator_bootstrap.html`, and CFT (Curator Forwardtest, `--run-dir data/curator_runs/forward-ft`) to `docs/index.html`. Each re-replays its FIXED curation JSONs through the optimizer and re-renders, live-extending the equity curve. They live in this job because they read the same-day `snapshots.csv` and must run after it. The old real-holdings `cli dashboard` step is RETIRED; CFT is the recommendation of record and owns `index.html`.
3. `scripts/review_curation.sh`, **Sunday at 19:00 local, plus a Monday 08:00 catch-up firing**: the biweekly curation, one `scripts/run_bootstrap_curator.py --if-due --grid-anchor "$GRID_ANCHOR"` per paper portfolio (CBS, then CFT with `--forward-only --blend-backtest-news --report`). This is the only cron step that calls the curator LLM. Sunday 19:00 lands AFTER that day's 18:30 news pull (which runs weekends too), so the curator reads same-day news and the report plus refreshed dashboard are ready before the user rebalances Monday morning; the Monday 08:00 firing only catches an overslept Sunday. `--if-due` self-gates to the profile's `rebalance_period` (biweekly), so the extra firing is a no-op and the cadence stays profile-driven (no cron edit to change it). **Cadence vs phase**: the profile owns the cadence; `GRID_ANCHOR` (a Sunday, set at the top of `review_curation.sh`) owns the phase. CBS and CFT are seeded on different dates (Saturday- and Wednesday-phased respectively) and used to fall on alternating weeks; anchoring both to the same Sunday makes them co-fire. `--grid-anchor` re-phases only FUTURE dates: any pre-anchor date that already has a curation JSON keeps its slot, so re-phasing re-curates nothing and leaves the replayed equity curves untouched. Each Sunday curation date maps to the following Monday's rebalance boundary in the replay, so the paper portfolios rebalance the same morning the user does. The old live `cli review --if-due` step is RETIRED (2026-08-01) because it curated the root `watchlist.csv` in parallel with CFT and produced a second, competing recommendation.

Users install them by pasting the block into `crontab -e` (README §6 has it, using a `PWR_path` variable; four job lines, since the curation has a Sunday slot and a Monday catch-up); `scripts/install_cron.sh` is an optional helper that adds all of them idempotently. All scripts resolve their own location, are pure-bash, log to `data/snapshot.log`, and work on macOS and Linux.

cron only fires while the machine is awake; missed runs do not auto-replay, with one exception: `snapshot` backfills any session inside its 7-day price window that `snapshots.csv` is missing, so a lost weekday heals on the next run (a backfilled row uses the CURRENT `holdings.csv` shares, so it is only right if the position did not change). Older gaps still need `--date YYYY-MM-DD`.

`snapshot` also refuses to write a date it cannot fully price, and retries with backoff first (`portfolio._yf_download`). yfinance rate-limits by IP and signals it with an empty frame rather than an exception, so the old `value = 0.0` fallback turned a transient throttle into a permanent phantom crash in `total_value` (2026-08-31, three of four holdings zeroed, $64,700 to $16,392, no error raised). A missing date is recoverable; a zeroed one corrupts every chart downstream. The same throttling can empty plot 15's per-ticker price popup, which is why a bar with no embedded history is daggered and says so on click instead of silently doing nothing.

Four dashboards are cron-refreshed: `docs/index.html` (CFT) and `docs/curator_bootstrap.html` (CBS) by `price_snapshot.sh` at 16:30, `docs/retrieval_bootstrap.html` (RBS) and `docs/retrieval_forward.html` (RFT) by `news_pull.sh` at 18:30. Every other page under `docs/` (the backtest, sweep, diagnostics, and pool-browser renders) is manual. cron itself does not push, so `git status` shows the four modified after each run; the `scripts/autopush_docs.sh` Stop hook publishes them at the end of a Claude Code session, and `git add docs/ && git commit && git push` does it by hand otherwise. The push (touching `docs/**`) fires the `Deploy dashboard to Pages` GitHub Action, which deploys the site; there is no separate publish step.

No cron runs the root-portfolio `cli recommend` or `cli review`. The curation job curates the two paper portfolios through `scripts/run_bootstrap_curator.py`, which writes only inside `data/curator_runs/<run>/`, so root `watchlist.csv` and `data/recommendations.csv` change only on a deliberate manual `review`/`recommend`. There is no daily `recommend` either: the curator's add/remove decisions are the only thing changing the optimizer's universe between rebalances, so a between-review `recommend` would produce a near-duplicate row.
