...
The current 5y backtest (Mar 2021 → Mar 2026, starter watchlist of AAPL/MSFT/GOOGL/NVDA/SPY, 21 quarterly curator calls, profile defaults `λ=0.5 / lookback=1.5y / concentration_cap=0.7 / max_watchlist_size=8`) lifts realized return to **+1267.4%** vs **+328.7%** for an equal-weight buy-and-hold of the starter and **+75.7%** SPY, with a −44.5% max drawdown. NVDA is in the buy-and-hold starter so the curator doesn't get credit for picking the obvious 5y winner — the +34.9pp/yr remaining lift comes from the curator's other thematic adds (nuclear, robotics, rockets, quantum) plus the optimizer's quarterly re-weighting. The backtest models a realistic execution lag (`t_update_days=1`, the CLI default): each rebalance is decided on the rebalance date's close but the trade lands the next session, since a live user runs a review and only places the order later. The result is insensitive to the lag — the curator return is flat-to-slightly-higher across a 0-to-3-session sweep (the +1267.4% at t=1 vs +1259.1% at t=0), so the edge is not a fast-execution artifact a real user would miss. Rendered at [docs/backtest_curator.html](https://joehahn.github.io/portfolio-wave-rider/backtest_curator.html). The full setup and reproducibility notes live in `REFERENCE.md` and `data/backtest_curator_5y/report.md`. The canonical run dir is `data/curator_runs/5y-sweep-cap08/`; `5y-quarterly/` is the historical record from when the default was cap=12.
