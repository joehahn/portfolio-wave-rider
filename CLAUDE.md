...
The current backtest window is **post-COVID, normal-regime** (2022-03-31 → 2025-10-31, ~3.6y, 15 quarterly curator calls), deliberately dropping the distorted 2020–2021 stimulus melt-up and ending just before the late-2025 Iran-war runup. Window dates and execution lag are set in `investor_profile.md`'s `backtest` section. Starter watchlist AAPL/MSFT/GOOGL/NVDA/SPY, profile defaults `λ=0.5 / lookback=1.5y / concentration_cap=0.7 / max_watchlist_size=8`. It lifts realized return to **+313.6%** vs **+187.3%** for an equal-weight buy-and-hold of the starter and **+58.7%** SPY, with a −43.0% max drawdown and +48.5% annualized. NVDA is in the buy-and-hold starter so the curator doesn't get credit for the obvious AI winner — the +14.3pp/yr remaining lift comes from the curator's thematic adds (rockets, robotics, quantum, nuclear, defense) plus the optimizer's quarterly re-weighting. The backtest models a realistic next-session execution lag (`t_update_days=1`, the CLI default): on this shorter window the lag is **material** — it trims the curator's edge (the optimistic same-close run, `t_update_days=0`, would be +345.8%), so the published **+313.6%** is the conservative figure a normally-paced human should expect, not a fast-execution upper bound. (On the longer 2021–2026 window the lag was near-noise; here the smaller rebalance count and sharp thematic entries make timing matter more.) Rendered at [docs/backtest_curator.html](https://joehahn.github.io/portfolio-wave-rider/backtest_curator.html). Setup and reproducibility live in `REFERENCE.md` and `data/backtest_curator_postcovid/report.md`. The published run dir is `data/curator_runs/postcovid/`; the longer 2021–2026 run (+1267% over its window) is preserved in `data/curator_runs/5y-sweep-cap08/` + `data/backtest_curator_5y/` for reference.
