...
The backtest runs the **post-COVID, normal-regime** window (2022-03-31 → 2025-10-31, ~3.6y, 15 quarterly curator calls; window set in `investor_profile.md`'s `backtest` section), starter watchlist AAPL/MSFT/GOOGL/NVDA/SPY. **Two optimizer configurations matter and must not be confused:**

- **Live recommend path — robust config** (`λ=0.5 / lookback=1.5y / concentration_cap=0.7`, the profile's `financial_model` + top-level defaults). This is what `/review-portfolio` uses with real money. It backtests at **+437.6%** vs +187.3% equal-weight buy-and-hold and +58.7% SPY (−43.2% max DD, +59.8% ann; +25.6pp/yr lift over B&H) — a diversified result (ends 70% RKLB / 30% WELL) whose lift still rests largely on one position (see caveats).
- **Published dashboard — aggressive config** (`λ=0.33 / lookback=0.5y / concentration_cap=1.0`, set as **backtest-only overrides** in the `backtest` section so they never touch live). These are the sweep-optimal knobs; they backtest at **+817%** but the optimizer ends up **~100% in a single name (RKLB)** — maximal overfit to the one 2025 winner, −55% max DD. `docs/backtest_curator.html` renders this config; treat it as the aggressive ceiling, not the honest expectation.
