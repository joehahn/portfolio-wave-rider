...
**Published backtest**: `data/curator_runs/proto-mws20/` (+ `_backtest/report.md`), rendered to `docs/backtest_gkg_3yr_kimi.html` (CBT) by `scripts/_finish_compounder_dbs.py`. Curator **+619.9%** ($50K → $360.0K, +93.2% annualized, −34.4% max drawdown) vs **+115.2%** equal-weight buy-and-hold of the starter and **+70.8%** SPY, from 28 adds and 11 removes across the 79 calls. **Known discrepancy:** that run was curated at `max_watchlist_size=20` while the profile's live default is 16, so the headline is not a pure profile-default replay.
