...
- **Live recommend path — robust config** (`λ=0.5 / lookback=1.5y / concentration_cap=0.7`, the profile's `financial_model` + top-level defaults). This is what `/review-portfolio` uses with real money. It backtests at **+313.6%** vs +187.3% equal-weight buy-and-hold and +58.7% SPY (−43.0% max DD, +48.5% ann; +14.3pp/yr lift over B&H) — a diversified result whose lift still rests largely on one position (see caveats).
- **Published dashboard — aggressive config** (`λ=0.33 / lookback=0.5y / concentration_cap=1.0`, set as **backtest-only overrides** in the `backtest` section so they never touch live). These are the sweep-optimal knobs; they backtest at **+910%** but the optimizer ends up **~100% in a single name (RKLB)** — maximal overfit to the one 2025 winner, −48% max DD. `docs/backtest_curator.html` renders this config; treat it as the aggressive ceiling, not the honest expectation.
