...
subject to ∑ᵢ wᵢ = 1 (weights sum to one) and 0 ≤ wᵢ ≤ concentration_cap. The first term `μᵀw` is the portfolio's expected return (the weighted average of per-ticker expected returns); the second term `wᵀΣw` is the portfolio's return variance, scaled by `λ` to act as a risk penalty. The two inputs estimated from price history are `μ` (the per-ticker expected-return vector — annualized mean of daily log returns over the **price-history lookback** (which default 1.3y in `investor_profile.md`)) and `Σ` (the ticker × ticker covariance matrix). `w` is the weight vector the optimizer is solving for. `λ` (risk aversion) trades expected return against variance: at `λ → 0` the variance term drops out and weight piles into the highest-`μ` tickers up to the cap; at `λ ≫ 1` the variance penalty dominates and the solution approaches min-variance (heavy in bonds and cash). The profile default `λ = 1` sits in between. This is the standard Markowitz mean-variance formulation (Markowitz 1952, *Portfolio Selection*, Journal of Finance 7:77-91), still the textbook starting point for portfolio construction because it captures the central return-vs-risk tradeoff in a single closed-form quadratic program. See [GLOSSARY.md](GLOSSARY.md) for the full definitions.
