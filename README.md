...
- **Subagents** A Subagent uses an LLM with narrow lists of allowed tools. Each one manages its own context window so the work it does (news reading, report writing) doesn't crowd the main conversation. Calls are fire-and-forget: spawn, run, return one message back, then the subagent's context disappears. Any state that needs to persist across calls is stored in files in the `data/` directory. This project has subagents, the [`watchlist-curator`](.claude/agents/watchlist-curator.md) that reads the news and proposes portfolio adds and removes, and the [`report-writer`](.claude/agents/report-writer.md) that writes the monthly report by synthesizing the curator's output and the optimizer's numbers.
