...
- **On demand, you decide** — you run `/run-backtest` in Claude Code. Walk-forward monthly-rebalance backtest of the optimizer over a 12-month window, applying the wave-stage tilts that were known at the start of each historical month. The run itself makes no LLM calls — it reads the pre-computed tilts from `data/wave_history.csv` (produced earlier by the news-researcher subagent during prior `/review-portfolio` runs or the as-of-date pilot rebuild). Auto-renders both the local backtest dashboard and the public-demo backtest page.
