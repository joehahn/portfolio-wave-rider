...
- **[Curator Backtest (CBT)](https://joehahn.github.io/portfolio-wave-rider/backtest_gkg_3yr_kimi.html)**: this dashboard inspects ouptut of the AI curator as it scans the 3 years of backtest news to make periodic changes to the ticker watchlist that is then fed into the portfolio optimizer, with this dashboard monitoring the ticker recommendations and portfolio valuations over time.
...
Each portfolio rebalance after one iteration of the following loop. The Curator is the only part of this solution that relies on AI for judgment calls, everything else is deterministic Python.
