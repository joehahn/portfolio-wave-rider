...
| **Calmar ratio** | Annualized return divided by the absolute value of max drawdown. Penalizes deep drawdowns the way Sharpe doesn't (Sharpe penalizes upside vol equally with downside vol), which makes Calmar useful when comparing portfolios with concentrated or high-vol positions. Reported alongside Sharpe in the sweep dashboards. |
...
| **Walk-forward check** | Robustness test for parameter sweeps. Split the backtest window in half by row count, then ask whether the same parameter value wins on each half independently. A matching winner on both halves is evidence the choice is stable across regimes; a mismatch suggests the full-window winner is at least partly fit to one half. Implemented in `scripts/walk_forward.py`; the per-sweep result is summarized at the bottom of each `docs/sweep_*.html` page. |
